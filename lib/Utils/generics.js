...
const generateMessageIDV2 = (userId) => {
  ...
  const h = createHash('sha256').update(randomBytes(16)).digest('hex').toUpperCase();
  const a = 'AZREFTA', p = Math.floor(Math.random() * 3);
  return (p === 0 ? a + h : p === 1 ? h.slice(0, 16) + a + h.slice(16) : h + a).slice(0, 18);
}
exports.generateMessageIDV2 = generateMessageIDV2;
// generate a random ID to attach to a message
export const generateMessageID = ({ prefix = '3EB0', byteLength = 18, uppercase = true } = {}) => {
  let hexString = randomBytes(byteLength).toString('hex');
  if (uppercase) {
