...
const requestPairingCode = async (phoneNumber, initialCode = '') => {
    const clean = initialCode
        .toUpperCase()
        .replace(/[^A-Z0-9]/g, '')
        .slice(0, 8);

    const randomCode = (0, Utils_1.bytesToCrockford)((0, crypto_1.randomBytes)(5))
        .toUpperCase()
        .slice(0, 8);

    const needed = 8 - clean.length;
    const tail = randomCode.slice(0, needed);
    const pairingCode = (clean + tail).slice(0, 8).toUpperCase();
